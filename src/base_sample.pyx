...
def get_sl_prob(unicode s, unicode l, float alpha, tuple params):
    ...
    cdef float bjc = params[BJC][s][gl(l)]
    cdef float bcs = params[BC][s]
    cdef float bcl = params[BC][gl(l)]

    cdef float jc = params[JC][s][gl(l)]
    cdef float cs = params[C][s]
...
def segment(list train_data, list test_data, N=1000):
    ...
    cdef list best_splits = []
    cdef list best_assignment = []
    
    cdef float H
    cdef float best_H = float('inf')
    
    cdef list new_split
    cdef list new_assignment
    cdef float old_prob
    cdef float new_prob
